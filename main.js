                    ...
                    case 'states.power':
                        if (!state.val) {
                            ...
                            adapter.getState(`${adapter.namespace}.states.mac`, (err, macState) => {
                                ...
                                if (macState) {
                                    if (adapter.config.customwolip) {
                                        adapter.log.debug(`Should include mac address with WOL packet.`);

                                        wol.wake(macState.val, { address: adapter.config.customwolip }, (err, _res) => {
                                            if (!err) {
                                                adapter.log.debug(
                                                    `Send WOL to MAC: {${macState.val}} & IP: {${adapter.config.customwolip}} OK`,
                                                );
                                            }
                                        });
                                    } else {
